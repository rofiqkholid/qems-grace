...
<div class="lg:ml-20 min-h-screen flex flex-col bg-slate-50 overflow-x-hidden max-w-full">
    ...
    <main class="flex-1 px-4 py-2 lg:px-6 lg:py-3">
        ...
        <!-- Main Card (Monthly Summary Table) -->
                    ...
                    <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                        ...
                        <div class="min-w-[160px]">
                            <x-searchable-select
                                ...
                                :initialOptions="collect($departments)->map(fn($dept) => ['id' => is_object($dept) ? $dept->Key1 : $dept, 'name' => is_object($dept) ? $dept->Key1 : $dept])->toArray()"
                                hideLabel="true" />
                        </div>
    ...
    $(document).ready(function() {
        ...
        @php
            ...
            if (!empty($selectedDept)) {
                $foundDept = collect($departments)->first(function($d) use ($selectedDept) {
                    return (is_object($d) ? $d->Key1 : $d) === $selectedDept;
                });
                if ($foundDept) {
                    $deptName = is_object($foundDept) ? $foundDept->Key1 : $foundDept;
                }
            }
