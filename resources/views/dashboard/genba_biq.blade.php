...
@section('title', 'Genba BIQ Dashboard')
...
<script>
    ...
    function loadDataCards(yearMonth) {
        $.ajax({
            url: "{{ route('dashboard.biq.data_cards') }}",
            data: {
                yearMonth: yearMonth
            },
            type: "GET",
    ...
    $(document).ready(function() {
        table = $('#findingsTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('dashboard.biq.table') }}", // Uses DashboardController table method (no delete button)
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
