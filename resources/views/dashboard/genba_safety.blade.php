...
@section('title', 'Genba Safety Dashboard')
...
<script>
    ...
    function loadDataCards(yearMonth) {
        $.ajax({
            url: "{{ route('dashboard.safety.data_cards') }}",
            data: {
                yearMonth: yearMonth
            },
            type: "GET",
    ...
    function loadDeptChart(yearMonth) {
        $.ajax({
            url: "{{ route('dashboard.safety.chart_data', ':yearMonth') }}".replace(':yearMonth', yearMonth),
            type: "GET",
            dataType: "json",
            success: function(response) {
    ...
    $(document).ready(function() {
        table = $('#findingsTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('dashboard.safety.table') }}", // Uses DashboardController table method (no delete button)
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
