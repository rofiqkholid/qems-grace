...
@section('title', 'Line Checked Master')
...
<div class="lg:ml-20 min-h-screen flex flex-col bg-slate-50">
    ...
    <main class="flex-1 p-6">
        <!-- Page Title -->
        <div class="mb-6">
            <h1 class="text-lg sm:text-2xl font-bold text-slate-800">Line Checked Master</h1>
            <p class="text-slate-500 text-xs sm:text-sm mt-1">Manage process line checked definitions and their associated details</p>
        </div>
        ...
        <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            ...
            <div class="p-4 sm:p-6 border-b border-slate-200 bg-slate-50/50">
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            <input type="text" id="searchInput" placeholder="Search Process or Area Name..."
            ...
            <div class="p-6">
                <table id="lineCheckedTable" class="qms-table w-full min-w-[800px]">
                    <thead>
                        <tr>
                            <th class="w-[5%] text-center">No</th>
                            <th class="w-[40%]">Process</th>
                            <th class="w-[40%]">Area Name</th>
            ...
            <x-data-table tableId="lineCheckedTable" />
        </div>
    </main>
...
<div id="createModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-md transform transition-all">
            ...
            <form id="createForm" action="{{ route('master.line_checked.store') }}" method="POST">
                @csrf
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Process</label>
                        <input type="text" name="process" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="Enter process">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Area Name</label>
                        <input type="text" name="area_name" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="Enter area name">
                    </div>
                </div>
...
<div id="editModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-md transform transition-all">
            ...
            <form id="editForm" action="{{ route('master.line_checked.update') }}" method="POST">
                @csrf
                <input type="hidden" name="sys_id" id="edit_sys_id">
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Process</label>
                        <input type="text" name="process" id="edit_process" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Area Name</label>
                        <input type="text" name="area_name" id="edit_area_name" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all">
                    </div>
                </div>
                <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 rounded-b-xl">
...
<script>
    $(document).ready(function() {
        var table = $('#lineCheckedTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('master.line_checked.table') }}",
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
    ...
    $(document).on('keyup', '#searchInput', function() {
        ...
        searchTimer = setTimeout(function() {
            $('#lineCheckedTable').DataTable().search($(self).val()).draw();
        }, 500);
    });
    ...
    function executeDelete() {
        ...
        $.ajax({
            url: "{{ route('master.line_checked.delete') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                sys_id: id
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#lineCheckedTable').DataTable().ajax.reload();
                } else {
