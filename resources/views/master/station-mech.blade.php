...
@section('title', 'Station Mech Master')
...
<div class="lg:ml-20 min-h-screen flex flex-col bg-slate-50">
    ...
    <main class="flex-1 p-6">
        <!-- Page Title -->
        <div class="mb-6">
            <h1 class="text-lg sm:text-2xl font-bold text-slate-800">Station Mech Master</h1>
            <p class="text-slate-500 text-xs sm:text-sm mt-1">Manage station mechanics, area, and detail area definitions</p>
        </div>
        ...
        <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
            ...
            <div class="p-4 sm:p-6 border-b border-slate-200 bg-slate-50/50">
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            <input type="text" id="searchInput" placeholder="Search Area or Detail Area..."
                    ...
                    <button onclick="openCreateModal()" class="shrink-0 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg flex items-center gap-1.5 sm:gap-2 transition-colors text-xs sm:text-sm font-medium">
                        ...
                        <span>Add Station Mech</span>
                    </button>
                </div>
            </div>

            <div class="p-6">
                <table id="stationMechTable" class="qms-table w-full min-w-[800px]">
                    <thead>
                        <tr>
                            <th class="w-[5%] text-center">No</th>
                            <th class="w-[40%]">Area</th>
                            <th class="w-[40%]">Detail Area</th>
            ...
            <x-data-table tableId="stationMechTable" />
        </div>
    </main>
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-md transform transition-all">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-bold text-slate-800">Add Station Mech</h3>
                <button onclick="closeCreateModal()" class="text-slate-400 hover:text-slate-600">
                    <i class="fa-solid fa-xmark text-xl"></i>
                </button>
            </div>
            <form id="createForm" action="{{ route('master.station-mech.store') }}" method="POST">
                @csrf
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Area</label>
                        <input type="text" name="area" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="Enter area (e.g. Assembly)">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Detail Area</label>
                        <input type="text" name="detail_area" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" placeholder="Enter detail area (e.g. PSW-HPM)">
                    </div>
                </div>
...
<div id="editModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-md transform transition-all">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-bold text-slate-800">Edit Station Mech</h3>
                <button onclick="closeEditModal()" class="text-slate-400 hover:text-slate-600">
                    <i class="fa-solid fa-xmark text-xl"></i>
                </button>
            </div>
            <form id="editForm" action="{{ route('master.station-mech.update') }}" method="POST">
                @csrf
                <input type="hidden" name="sys_id" id="edit_sys_id">
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Area</label>
                        <input type="text" name="area" id="edit_area" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1">Detail Area</label>
                        <input type="text" name="detail_area" id="edit_detail_area" required class="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all">
                    </div>
                </div>
                <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3 rounded-b-xl">
    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl w-full max-w-sm transform transition-all">
            <div class="p-6 text-center">
                ...
                <p class="text-slate-500 text-sm">Are you sure you want to delete this station mech? This action cannot be undone.</p>
            </div>
...
<script>
    $(document).ready(function() {
        var table = $('#stationMechTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('master.station-mech.table') }}",
                type: 'POST',
                data: function(d) {
                    d._token = "{{ csrf_token() }}";
    ...
    $(document).on('keyup', '#searchInput', function() {
        ...
        searchTimer = setTimeout(function() {
            $('#stationMechTable').DataTable().search($(self).val()).draw();
        }, 500);
    });
    ...
    function handleEdit(btn) {
        ...
        const detailArea = btn.getAttribute('data-detailarea');
    ...
    function executeDelete() {
        ...
        $.ajax({
            url: "{{ route('master.station-mech.delete') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                sys_id: id
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#stationMechTable').DataTable().ajax.reload();
                } else {
