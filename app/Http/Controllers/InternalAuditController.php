...
class InternalAuditController extends Controller
{
    ...
    {
        $query = DB::table('CsAuditSchedule as a')
            ->select('a.*');
        ...
        if ($request->has('search') && !empty($request->search['value'])) {
            ...
            $query->where(function($q) use ($searchValue) {
                $q->where('a.agenda_name', 'LIKE', "%{$searchValue}%")
                  ->orWhere('a.auditor_niks', 'LIKE', "%{$searchValue}%")
                  ->orWhere('a.auditee_dept', 'LIKE', "%{$searchValue}%");
            });
        }

        $totalData = DB::table('CsAuditSchedule')->count();
        ...
        foreach ($posts as $post) {
            ...
            if ($post->status === 'Scheduled') {
                $action .= '<a href="' . route('internal_audit.conduct', $post->id) . '" class="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200" title="Conduct Audit">
                ...
                $action .= '<button type="button" disabled class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 text-slate-400 cursor-not-allowed" title="Audit Completed">
            ...
            if ($hasDeletePermission) {
                $action .= '<button type="button" onclick="deleteAuditSchedule(' . $post->id . ')" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 transition-all duration-200" title="Delete Schedule">
            ...
            $auditors = array_filter(preg_split('/\s*[,&]\s*/', $post->auditor_niks));
            ...
            $data[] = [
                'no' => $no,
                'id' => $post->id,
                'agenda_name' => $post->agenda_name,
                'schedule_date' => Carbon::parse($post->schedule_date)->format('d M Y'),
    ...
    public function storeSchedule(Request $request)
    {
        ...
        try {
            DB::table('CsAuditSchedule')->insert([
                'agenda_name' => $request->agenda_name,
                'schedule_date' => $request->schedule_date,
                'auditor_niks' => $request->auditor_niks,
            ...
            return response()->json(['success' => true, 'message' => 'New schedule added successfully.']);
        } catch (\Exception $e) {
    ...
    {
        $schedule = DB::table('CsAuditSchedule')->where('id', $schedule_id)->first();
        if (!$schedule) {
        ...
        return view('activity.form-checksheet-intr.activity_intr_form', compact('schedule', 'items'));
    }
    ...
    public function submitAudit(Request $request)
    {
        ...
        try {
            ...
            $headerId = DB::table('CsAuditHeader')->insertGetId([
    ...
    public function deleteSchedule($id)
    {
        ...
        try {
            DB::table('CsAuditSchedule')->where('id', $id)->delete();
