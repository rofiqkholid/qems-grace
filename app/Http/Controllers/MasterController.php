...
class MasterController extends Controller
{
        ...
        $response = [
            ...
            "data" => $data->map(function ($item, $key) use ($request, $canDelete) {
                ...
                return [
                    ...
                    "arrow_target" => $item->arrow_target,
                    "result" => $item->result,
                    "action" => '<div class="flex items-center justify-start gap-2">
                                <button type="button" title="Edit" class="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200"
                                     onclick="handleEdit(this)"
                                     ...
                                     data-arrow_target="' . htmlspecialchars($item->arrow_target ?? '') . '"
                                     data-result="' . htmlspecialchars($item->result ?? '') . '">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path opacity="0.3" d="M10 4H21C21.6 4 22 4.4 22 5V7H10V4Z" fill="currentColor"></path>
                                    <path opacity="0.3" d="M10.3 15.3L11 14.6L8.70002 12.3C8.30002 11.9 7.7 11.9 7.3 12.3C6.9 12.7 6.9 13.3 7.3 13.7L10.3 16.7C9.9 16.3 9.9 15.7 10.3 15.3Z" fill="currentColor"></path><path d="M10.4 3.60001L12 6H21C21.6 6 22 6.4 22 7V19C22 19.6 21.6 20 21 20H3C2.4 20 2 19.6 2 19V4C2 3.4 2.4 3 3 3H9.20001C9.70001 3 10.2 3.20001 10.4 3.60001ZM11.7 16.7L16.7 11.7C17.1 11.3 17.1 10.7 16.7 10.3C16.3 9.89999 15.7 9.89999 15.3 10.3L11 14.6L8.70001 12.3C8.30001 11.9 7.69999 11.9 7.29999 12.3C6.89999 12.7 6.89999 13.3 7.29999 13.7L10.3 16.7C10.5 16.9 10.8 17 11 17C11.2 17 11.5 16.9 11.7 16.7Z" fill="currentColor"></path>
                                </svg>
                                <button type="button" title="Formula" class="w-10 h-10 flex items-center justify-center rounded-xl bg-amber-50 text-amber-500 hover:bg-amber-100 hover:text-amber-600 transition-all duration-200"
                                    ...
                                    <i class="fa-solid fa-square-root-variable text-base"></i>
                                </button>' . 
                                ($canDelete ? '
                                <button type="button" title="Delete" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 transition-all duration-200" 
                                    id="btn_delete_' . ($start + $key + 1) . '" 
    ...
    public function store_kpi_list(Request $request)
    {
        $request->validate([
            ...
            'arrow_target' => 'required|in:Down,Up',
            'result' => 'required|in:Total,Average',
        ]);
        ...
        try {
            $insertedId = DB::table('KPIList')->insertGetId([
                ...
                'arrow_target' => $request->arrow_target,
            ...
            if ($request->has('history') && is_array($request->history)) {
                foreach ($request->history as $year => $data) {
                    DB::table('KPIAchievementHistory')->insert([
                        ...
                        'unit' => $data['unit'] ?? null,
                        'status_achievement' => $data['status_achievement'] ?? null,
                        'created_at' => \Carbon\Carbon::now(),
                        'updated_at' => \Carbon\Carbon::now()
                    ]);
                }
    ...
    public function station_mech_table(Request $request)
    {
        $query = DB::table('GenbaStationMech')->orderBy('SysID', 'desc');
        ...
        if ($request->has('search') && !empty($request->search['value'])) {
            ...
            $query->where(function ($q) use ($searchValue) {
                $q->where('Area', 'LIKE', "%{$searchValue}%")
                    ->orWhere('DetailArea', 'LIKE', "%{$searchValue}%");
            });
        }

        // Total records
        $totalRecords = DB::table('GenbaStationMech')->count();
        $filteredRecords = $query->count();
                ...
                return [
                    ...
                    "action" => '<div class="flex items-center justify-center gap-2">
                                <button type="button" title="Edit" class="w-10 h-10 flex items-center justify-center rounded-xl bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-600 transition-all duration-200"
                                    ...
                                    data-detailarea="' . htmlspecialchars($item->DetailArea ?? '', ENT_QUOTES) . '">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path opacity="0.3" d="M10 4H21C21.6 4 22 4.4 22 5V7H10V4Z" fill="currentColor"></path>
                                    <path opacity="0.3" d="M10.3 15.3L11 14.6L8.70002 12.3C8.30002 11.9 7.7 11.9 7.3 12.3C6.9 12.7 6.9 13.3 7.3 13.7L10.3 16.7C9.9 16.3 9.9 15.7 10.3 15.3Z" fill="currentColor"></path><path d="M10.4 3.60001L12 6H21C21.6 6 22 6.4 22 7V19C22 19.6 21.6 20 21 20H3C2.4 20 2 19.6 2 19V4C2 3.4 2.4 3 3 3H9.20001C9.70001 3 10.2 3.20001 10.4 3.60001ZM11.7 16.7L16.7 11.7C17.1 11.3 17.1 10.7 16.7 10.3C16.3 9.89999 15.7 9.89999 15.3 10.3L11 14.6L8.70001 12.3C8.30001 11.9 7.69999 11.9 7.29999 12.3C6.89999 12.7 6.89999 13.3 7.29999 13.7L10.3 16.7C10.5 16.9 10.8 17 11 17C11.2 17 11.5 16.9 11.7 16.7Z" fill="currentColor"></path>
                                </svg>
    ...
    public function store_station_mech(Request $request)
    {
        $request->validate([
            'area' => 'required|string|max:100',
            'detail_area' => 'required|string|max:200',
        ]);

        try {
            DB::table('GenbaStationMech')->insert([
                'Area' => $request->area,
                'DetailArea' => $request->detail_area,
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            ...
            return redirect()->route('master.station-mech')->with('success', 'Data added successfully.');
        } catch (\Exception $e) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
    ...
    public function update_station_mech(Request $request)
    {
        $request->validate([
            'sys_id' => 'required',
            'area' => 'required|string|max:100',
        ...
        try {
            ...
            return redirect()->route('master.station-mech')->with('success', 'Data updated successfully.');
        } catch (\Exception $e) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
    ...
    public function delete_station_mech(Request $request)
    {
        $request->validate([
            'sys_id' => 'required',
        ]);
        ...
        try {
            DB::table('GenbaStationMech')->where('SysID', $request->sys_id)->delete();
            return response()->json(['success' => true]);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()]);
        }
